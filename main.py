...
ardila_dicom = ardila.merge(cohort, left_on=['pid', 'scandate'], right_on=['pid', 'year'])
if args.limit:
...
logger = logging.basicConfig(filename='log/main.log', filemode='a', format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s', level=logging.DEBUG)
...
if phase == 'test':
    ...
    nifti_scores = np.load('./data/nlst/nifti_ardila_scores_n500.npy')
    max_dicom_scores = np.max(dicom_scores, axis=1)[:500]
    max_nifti_scores = np.max(nifti_scores, axis=1)
    ardila_dicom['sybil_dicom'] = max_dicom_scores
    ardila_dicom['sybil_nifti'] = max_nifti_scores
    dicom_metrics = bootstrap(ardila_dicom, lambda x: roc_auc_score(x['lung_cancer'], x['sybil_dicom']), n=1000)
    nifti_metrics = bootstrap(ardila_dicom, lambda x: roc_auc_score(x['lung_cancer'], x['sybil_nifti']), n=1000)
